...
async function getGraphToken(creds: TeamsCreds): Promise<string> {
  const now = Date.now();
  if (graphToken && now < graphTokenExpiresAt) {
    return graphToken;
  }
...
export class TeamsProjectChannelManager {
  ...
  load(): void {
    ...
    try {
      ...
      this.log(
        `[ecs-teams] corrupted persist file, starting fresh: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  private async provision(projectId: string): Promise<string | null> {
    ...
    try {
      ...
      this.log(`[ecs-teams] provision failed: ${err instanceof Error ? err.message : String(err)}`);
      return null;
    }
  }
