...
export class ProjectChannelManager {
  ...
  private async doProvision(projectId: string): Promise<ProjectChannelSet | null> {
    ...
    try {
      ...
      try {
        ...
        this.log(`[ecs] onProjectProvisioned callback error: ${String(cbErr)}`);
      }
      ...
      if (status === 403) {
        ...
        this.log(`[ecs] failed to provision channels for "${projectId}": ${String(err)}`);
      }
      return null;
    }
  }
  ...
  async archiveProject(projectId: string): Promise<boolean> {
    ...
    try {
      ...
      this.log(`[ecs] failed to archive channels for "${projectId}": ${String(err)}`);
    }
  ...
  private persist(): void {
    try {
      ...
      this.log(`[ecs] failed to persist project channels: ${String(err)}`);
    }
  }
}
